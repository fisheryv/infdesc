...
\begin{itemize}
\item \textbf{交流  } 首先，本书旨在帮助读者获得数学素养并能用数学的方式表达自己。这发生在许多层面上，例如，考虑以下表达式：
\[ \forall x \in \mathbb{R},\, [\neg(x = 0) \Rightarrow (\exists y \in \mathbb{R},\, y^2 < x^2)] \]
学完本书，你会自然而然地说出符号 $\forall$, $\in$, $\mathbb{R}$, $\neg$, $\Rightarrow$ 和 $\exists$ 的含义以及它们的精确定义。你也能够从整体上解释这个表达式的含义，并用简单的术语向他人解释其所表达的意思，而\textit{不是}使用一堆混乱的符号。你还能够证明它是正确的，并用清晰简明的方式与他人交流你的证明。

为了实现此目标，本书主要章节都在构建这类工具，而 \Cref{apxWriting} 会更加关注书写层面。

\item \textbf{探究  } 研究表明，人们在自己发现事物的过程中会学到更多。如果我将这个观点发挥到极致，那本书将是一片空白；然而，我的确认为将探究式学习的各个方面融入本书非常重要。

这一原则体现在本书散布的习题中，其中许多习题只是要求你证明给出的结果。许多读者会有挫败感，但这是有充分理由的：这些习题作为检查点，确保你对内容的理解足以继续前行。那种挫败感就是学习的感觉 --- 请拥抱它！

\item \textbf{策略  } 数学证明很像一个谜题。在证明的任意给定阶段，你都会有若干可用的定义、假设和结果，你必须使用你可以支配的逻辑规则将它们拼凑在一起。贯穿全书，尤其是前面的章节，当有用的证明策略出现时，我会不遗余力地将它们强调出来。
...
在继续讨论 \Cref{ptTopics} 中的主题之前，没有必要涵盖所有 \Cref{ptCoreConcepts} 的内容。事实上，穿插 \Cref{ptTopics} 中的内容可能是启发 \Cref{ptCoreConcepts} 中出现的许多抽象概念的有用方法。

下表显示了章节之间的依赖关系。除非另有说明，否则同一章中的前几节均被视为`基本'先决条件。
...
广义上讲，本书中的内容被分解为五类项目：定义、结论、备注、示例和练习。在 \Cref{apxWriting} 中，我们也有证明摘录。为了提高导航性，这些类别通过名称、颜色和符号进行区分，如下表所示。
...
本书采用 Creative Commons Attribution-ShareAlike 4.0 (\textsc{cc by-sa 4.0}) 许可授权。这意味着你可以分享本书，但前提是你要注明作者的姓名，并且本书的任何副本或衍生品都是在同一许可下发布的。
