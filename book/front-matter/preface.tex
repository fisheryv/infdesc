...
这个网站还包含本书不同版本间差别的信息，历史版本归档，以及使用 \LaTeX{} 的一些资源（另见 \Cref{apxLaTeX}）。

\subsection*{关于本书}

在典型的微积分课上，学生会学习链式法则，然后运用该法则去解一些给定的`链式法则问题'，比如计算 $\sin(1+x^2)$ 对 $x$ 的导数，或者解决涉及变化率的应用题。期望学生正确应用链式法则得出正确答案，并给出完整的步骤以确信结论正确。从这个意义上说，学生是数学的\textit{消费者}。他们被赋予链式法则这个要无条件接受的工具，然后使用这个工具来解决范围狭窄的问题。
...
数学的消费者只需要说出他们是如何使用工具找到答案的。而数学的生产者必须做更多事情：他们必须能够追踪定义和假设，以新颖有趣的方式拼凑事实，并对数学概念给出自己的定义。更重要的是，一旦他们这样做了，他们必须以其他人可以理解的方式交流他们的发现，并且必须说服其他人他们所做的是正确的、适当的和有价值的。

正是这种从数学消费到生产的转变指导了我用于设计和编写本书的原则。尤其是：

\begin{itemize}
\item \textbf{交流} 首先，本书旨在帮助读者获得数学素养并用数学表达自己。这发生在许多层次上，例如，考虑以下表达式：。
\[ \forall x \in \mathbb{R},\, [\neg(x = 0) \Rightarrow (\exists y \in \mathbb{R},\, y^2 < x^2)] \]
学完本书，你将能够直观地说出符号 $\forall$, $\in$, $\mathbb{R}$, $\neg$, $\Rightarrow$ 和 $\exists$ 的含义以及它们是如何被精确定义的。你也能够从整体上解释这个表达的含义，用简单的术语向另一个人解释它的意思而 \textit{不是}使用一堆混乱的符号，你能够证明它是正确的，并用清晰简明的方式与他人交流你的证明。
...
这一原则体现在本书中散布的习题，其中许多习题只是要求你证明给出的结果。许多读者会觉得这令人沮丧，但这是有充分理由的：这些习题作为检查点，确保你对材料的理解足以继续前行。那种挫败感就是学习的感觉——请拥抱它！

\item \textbf{策略  } 数学证明很像一个谜题。在证明的任意给定阶段，你都会有若干可用的定义、假设和结果，你必须使用你可以支配的逻辑规则将它们拼凑在一起。贯穿全书，尤其是前面的章节，当有用的证明策略出现时，我不遗余力地将它们强调出来。

\item \textbf{内容  } 如果你对要证明的结果没有任何概念，那么学习数学就没有多大意义。考虑到这一点，\Cref{ptTopics} 包含几个章节专门介绍纯数学中的一些主题领域，例如数论、组合学、分析和概率论。

\item \textbf{\LaTeX{}  } 数学排版的\textit{事实}标准是 \LaTeX{}。我认为数学家在有指导的情况下尽早学习 \LaTeX{} 这一点很重要，因此我在 \Cref{apxLaTeX} 中编写了一个简短的教程，并包含了所有新符号的 \LaTeX{} 代码，因为它在整本书中都有定义。
...
这本书不需要，而且，重点强调也\textit{不应该}，从前到后阅读。之所以选择这个材料顺序是为了使后面出现的材料仅依赖于前面出现的材料，但是按照材料的呈现顺序进行学习可能是一种相当枯燥的体验。

大多数介绍性纯数学课程至少涵盖符号逻辑、集合、函数和关系。这个材料是\Cref{ptCoreConcepts}的内容。此类课程通常涵盖纯数学的附加主题，具体主题取决于课程为学生准备的内容。考虑到这一点，\Cref{ptTopics} 介绍了纯数学的一系列领域，包括数论、组合学、集合论、实分析、概率论和阶论。

在继续讨论 \Cref{ptTopics} 中的主题之前，没有必要涵盖所有的 \Cref{ptCoreConcepts}。 事实上，穿插 \Cref{ptTopics} 中的材料可能是激发 \Cref{ptCoreConcepts} 中出现的许多抽象概念的有用方法。

下表显示了章节之间的依赖关系。除非另有说明，否则同一章节中的前几节应被视为`基本'先决条件。
...
广义上讲，本书中的材料被分解到五类项目中：定义、结论、备注、示例和练习。在 \Cref{apxWriting} 中，我们也有证明摘录。为了提高导航性，这些类别通过名称、颜色和符号进行区分，如下表所示。
...
这些项目根据它们所在的章节一一呈现出来——例如，\Cref{thmUniquenessofLimits} 在 \Cref{secCompletenessConvergence} 中。定义和定理（重要结果）出现在 \fbox{方框} 中。

你还会遇到符号 $\square$ 和 \nonproofqedsymbol{} 它们的含义如下：

\begin{itemize}
\item[$\square$] \textbf{证明结束  } 这是数学文档中的标准，通过画一个小正方形或写下`\textit{Q.E.D.}'（后者代表 \textit{quod erat demonstrandum}，在拉丁语表示 \textit{这是要展示的}。）表示证明结束。
\item[\nonproofqedsymbol] \textbf{条目结束  } 这\textit{不}是标准用法，将其包含在内只是为了帮助你识别项目结束，接下来是本书的主要内容。
\end{itemize}

一些小节标有符号 \optmarksymbol{}。这表明可以跳过该小节中的材料而不会产生严重的后果。
