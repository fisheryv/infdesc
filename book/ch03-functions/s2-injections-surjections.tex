...
\[ D = \left\{ x \in \mathbb{Q} \ \bigg|\  x = \frac{a}{2^n} \text{ 其中 } a \in \mathbb{Z} \text{ 且 } n \in \mathbb{N} \right\} \]
...
\[ d(k) = \begin{cases}
(m,n) & \text{如果 } k=2^m \cdot 3^n \text{ 其中 } m,n \in \mathbb{N} \\
(0,0) & \text{否则}
\end{cases} \]
...
\begin{cproof}
假设 $f$ 是单射的且 $X$ 非空。给定 $x_0 \in X$ --- 请注意，该元素存在是因为 $X$ 非空 --- 并定义 $g : Y \to X$ 为
\[ g(y) = \begin{cases} x & \text{如果 } y=f(x) \text{ 其中 } x \in X \\ x_0 & \text{否则} \end{cases} \]
$g$ 定义中唯一可能导致其无法明确定义的部分是当 $y=f(x)$ 其中 $x \in X$ 时的情况。$g$ 明确良好的原因正是 $f$ 的单射性：如果 $y=f(x)$ 其中 $x \in X$，则 $x \in X$ 的值对于 $y = f(x)$ 是唯一的。（事实上，如果 $a \in X$ 满足 $y=f(a)$，那么我们通过 $f$ 的单射性可得 $a=x$。）
...
\[ k(y) = \begin{cases}
\sqrt{y} & \text{如果 } 2n \le y < 2n+1 \text{ 其中 } n \in \mathbb{N} \\
-\sqrt{y} & \text{否则}
\end{cases} \]
