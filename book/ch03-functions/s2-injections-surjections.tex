...
\begin{itemize}
\item (\textbf{Injectivity}) 给定 $x,y \in D$ 且假设 $f(x)=f(y)$。则 $\frac{x}{2^k}=\frac{y}{2^k}$，因此 $x=y$。
\item (\textbf{Surjectivity}) 给定 $y \in D$。我们需要找到 $x \in D$ 使得 $f(x)=y$。很明显如果 $2^ky \in D$ 则我们有 \[ f(2^ky)=\frac{2^ky}{2^k}=y \]
因此只需证明 $2^ky \in D$ 即可。
