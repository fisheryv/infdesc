...
\[ f(1) = \mathsf{红色}, \quad f(2) = \mathsf{紫色}, \quad f(3) = \mathsf{绿色} \]
来定义函数
\[ f : \{ 1, 2, 3 \} \to \{ \mathsf{红色}, \mathsf{黄色}, \mathsf{绿色}, \mathsf{蓝色}, \mathsf{紫色} \} \]
请注意，此时函数已完全定义：我们知道定义域 $\{ 1, 2, 3 \}$ 中所有元素的值。值域的某些元素（$\mathsf{黄色}$ 和 $\mathsf{蓝色}$）未使用并不重要 --- 重要的是定义域中的每个元素都与值域中的唯一一个元素确切关联。
